...
const (
	...
	OperatorExpireTime = 10 * time.Second
	...
	SlowOperatorWaitTime = 120 * time.Minute
)
...
func (o *Operator) CheckTimeout() bool {
	...
	if o.kind&OpRegion != 0 {
		ret = o.status.CheckTimeout(SlowOperatorWaitTime)
		if ret {
			log.Info("Checktimeout failed", zap.Uint64("RegionID", o.regionID), zap.Uint64("Timeout", uint64(SlowOperatorWaitTime.Seconds())))
		}
		return ret
	}
	ret = o.status.CheckTimeout(FastOperatorWaitTime)
	if ret {
		log.Info("Checktimeout failed", zap.Uint64("RegionID", o.regionID), zap.Uint64("Timeout", uint64(FastOperatorWaitTime.Seconds())))
	}
	return ret
}
