...
func (s *schedulerTestSuite) TestScheduler(c *C) {
	...
	mustExec([]string{"-u", pdAddr, "scheduler", "config", "balance-hot-region-scheduler", "set", "disable-rw-scheduler", "read"}, nil)
	mustExec([]string{"-u", pdAddr, "scheduler", "config", "balance-hot-region-scheduler"}, &conf1)
	expected1["disable-rw-scheduler"] = "read"
	c.Assert(conf1, DeepEquals, expected1)
	// test compatibility
	for _, store := range stores {
